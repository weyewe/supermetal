Ext.define('AM.controller.GuaranteeReturnEntries', {
  extend: 'Ext.app.Controller',

  stores: ['GuaranteeReturnEntries', 'GuaranteeReturns'],
  models: ['GuaranteeReturnEntry'],

  views: [
    'sales.guaranteereturnentry.List',
    'sales.guaranteereturnentry.Form',
  ...
  init: function() {
    this.control({
      'guaranteereturnentrylist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'guaranteereturnentryform button[action=save]': {
        click: this.updateObject
      },
      'guaranteereturnentrylist button[action=addObject]': {
        click: this.addObject
      },
      'guaranteereturnentrylist button[action=editObject]': {
        click: this.editObject
      },
      'guaranteereturnentrylist button[action=deleteObject]': {
        click: this.deleteObject
      },

			// monitor parent(guarantee_return) update
			'guaranteereturnlist' : {
	...
	reloadStore : function(record){
		...
		var store = this.getGuaranteeReturnEntriesStore();
  ...
  addObject: function() {
    ...
    var view = Ext.widget('guaranteereturnentryform', {
			parentRecord : record 
		});
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					guarantee_return_id : parentRecord.get('id')
				},
			...
			var newObject = new AM.model.GuaranteeReturnEntry( values ) ;
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('guarantee_return_id');
		var list  = this.getList();
