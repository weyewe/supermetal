class Api::SalesReturnEntriesController < Api::BaseApiController
  ...
  def index
    ...
    @objects = @parent.active_sales_return_entries.joins(:sales_return, :sales_item, :delivery_entry).page(params[:page]).per(params[:limit]).order("id DESC")
  ...
  #  
  #   @parent = SalesReturn.find_by_id params[:sales_return_id]
  #   @object = SalesReturnEntries.create_by_employee(current_user, @parent,  params[:sales_return_entry] )  
  #   
  ...
  # def destroy
  #   @object = SalesReturnEntries.find(params[:id])
  #   @parent = @object.sales_return 
  ...
  def search
    ...
    if  selected_id.nil?
      @objects = SalesReturnEntries.joins(:template_sales_item, :sales_return ).where{ (template_sales_item.name =~ query )   & 
                                (is_deleted.eq false )  & 
                                (sales_return.customer_id.eq customer_id)
                              }.
                        page(params[:page]).
                        per(params[:limit]).
                        order("id DESC")
    else
      @objects = SalesReturnEntries.joins(:template_sales_item, :sales_return).where{ (id.eq selected_id)  & 
                                (is_deleted.eq false ) & 
                                (sales_return.customer_id.eq customer_id)
                              }.
                        page(params[:page]).
