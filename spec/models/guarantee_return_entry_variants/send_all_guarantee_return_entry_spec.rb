...
describe GuaranteeReturnEntry do
  before(:each) do
   ...
   @template_sales_item = @has_production_sales_item.template_sales_item 
    ...
    @premature_quantity_sent = 5 
    @premature_delivery_entry = DeliveryEntry.create_delivery_entry( @admin, @delivery,  {
        :quantity_sent => @premature_quantity_sent, 
        :quantity_sent_weight => "#{@premature_quantity_sent * 10}" ,
        :sales_item_id => @has_production_sales_item.id,
    ...
    @premature_delivery_entry.reload 
    @template_sales_item.reload
    ...
    @quantity_confirmed_premature =   @premature_delivery_entry.quantity_sent 
    @premature_delivery_entry.update_post_delivery(@admin, {
      :quantity_confirmed => @quantity_confirmed_premature ,
      :quantity_confirmed_weight =>  "#{@quantity_confirmed_premature*10 }",  
      :quantity_returned => 0 ,
      :quantity_returned_weight => '0' ,
      :quantity_lost => 0 
    })
  ...
  it 'should finalze delivery entry' do
    ...
    @premature_delivery_entry.is_finalized.should be_true
  end
  
  
  
  context "[Previously: sending post production item(normal) and production (premature)]: doing the guarantee return" do
    before(:each) do
    ...
    it 'should  create guarantee entry, receiving production. because we sent some' do
      # production quantity_sent = 5 
    ...
    context "post confirm guarantee return: the post production" do
      before(:each) do
        @gre_post_production = 3
        @gre_production = 1  
        @gre_production_repair = 0
    ...
    end   # "post confirm guarantee return: the post production" 
    
    context "post confirm guarantee return: the  production" do
      before(:each) do
        @gre_post_production = 0
        @gre_production = 1  
        @gre_production_repair = 2
        @guarantee_return_entry = GuaranteeReturnEntry.create_guarantee_return_entry( @admin, @guarantee_return ,  {
          :sales_item_id                  => @has_production_sales_item.id ,
          :quantity_for_post_production   => @gre_post_production,
          :quantity_for_production        => @gre_production,
        ...
        @initial_receivable_post_production_guarantee_return = @has_production_sales_item.receivable_guarantee_return_normal_post_production
        @initial_receivable_production_guarantee_return = @has_production_sales_item.receivable_guarantee_return_normal_production
        @guarantee_return.confirm(@admin)
        @template_sales_item.reload 
      ...
      it 'should confirm guarantee_return' do
        @guarantee_return.is_confirmed.should be_true 
      end
      
      it 'should confirm guarantee_return_entry' do
        @guarantee_return_entry.is_confirmed.should be_true 
      end
      
      it 'should NOT increase number of pending post production' do
        @final_pending_post_production = @template_sales_item.pending_post_production
        diff = @final_pending_post_production - @initial_pending_post_production
        diff.should == 0 
      end
      ...
      it 'should increase number of pending production repair' do
        ...
        diff.should  == @gre_production_repair 
      end
      
      it 'should not deduct number of receivable post production guarantee return'  do
        @final_receivable_post_production_guarantee_return = @has_production_sales_item.receivable_guarantee_return_normal_post_production
        diff =  @initial_receivable_post_production_guarantee_return - @final_receivable_post_production_guarantee_return
        diff.should == 0 
      end
      
      it 'should not deduct number of receivable  production guarantee return'  do
        @final_receivable_production_guarantee_return = @has_production_sales_item.receivable_guarantee_return_normal_production
        diff =  @initial_receivable_production_guarantee_return - @final_receivable_production_guarantee_return
        diff.should == @gre_production_repair + @gre_production
      end
    end   # "post confirm guarantee return: the  production"
